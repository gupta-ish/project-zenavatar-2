...
def main(cfg : DictConfig) -> None:
    all_pkls = glob.glob("./humanoidverse/data/AMASS/AMASS_Complete/ACCAD/**/*.npz", recursive=True)
    ...
    #     os.makedirs(f"humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/v1/singles", exist_ok=True)
    #     dumped_file = f"humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/v1/singles/{data_key}.pkl"
    #     logger.info(colored(f"Dumping to {dumped_file}", "green"))
    #     joblib.dump(all_data, dumped_file)
    # else:
    #     os.makedirs(f"humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/v1/", exist_ok=True)
    #     joblib.dump(all_data, f"humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/v1/amass_all.pkl")
    #     logger.info(colored(f"Dumping to humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/v1/amass_all.pkl", "green"))
    ...
    for key_name in key_names:
        ...
        if len(current_data) > 0:  # Only save if processing was successful
            os.makedirs(f"humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/accad_loco/singles", exist_ok=True)
            dumped_file = f"humanoidverse/data/motions/{cfg.robot.motion.humanoid_type}/accad_loco/singles/{key_name}.pkl"
            logger.info(colored(f"Dumping to {dumped_file}", "green"))
