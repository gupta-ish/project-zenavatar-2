...
class DecoupledLocomotionManipulationForcePolicy(DecoupledLocomotionManipulationForcePolicy):
    ...
    def run_motion_planning(self):
        ...
        # qpos = self.robot_state_data[:, 7:7+self.num_dofs]
            ...
            self.goal_pose_L_2 = pin.SE3(np.eye(3), np.array([self.goal_pose_L_1.translation[0], 0.14, self.goal_pose_L_1.translation[2]]))
            self.goal_pose_R_2 = pin.SE3(np.eye(3), np.array([self.goal_pose_R_1.translation[0], -0.14, self.goal_pose_L_1.translation[2]]))
    ...
    def handle_joystick_button(self, cur_key):
        ...
        if self.upper_body_controller:
            ...
            #     self.run_motion_planning()
                ...
                self.state_index()
                self.run_motion_planning()
                if self.MP_enabled == True and self.st > 3:
                    self.MP_enabled = False
                    self.st = 0
                    self.waypoint_index = 0
            ...
            if cur_key == "B+X":
                ...
                # self.test_ = True
                # self.lin_vel_command[0, 1] = 0.1 #-(self.wc_msg.lx if abs(self.wc_msg.lx) > 0.1 else 0.0) * self.stand_command[0, 0] * 0.5
            elif cur_key == "A+B":
