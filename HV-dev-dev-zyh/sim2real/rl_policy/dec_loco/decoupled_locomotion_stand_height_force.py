...
class DecoupledLocomotionStandHeightForcePolicy(DecoupledLocomotionStandHeightPolicy):
    def __init__(self, 
                 config, 
                 model_path, 
                 use_jit,
    ...
    def prepare_obs_for_rl(self, robot_state_data):
        ...
        if self.use_history:
            ...
            # self.logger.info(f"obs.shape: {obs.shape}")
        else:
            obs = np.concatenate([self.last_policy_action, 
                                self.apply_force*0.1,
                                base_ang_vel*0.25, 
...
if __name__ == "__main__":
    ...
    policy = DecoupledLocomotionStandHeightForcePolicy(config=config, 
