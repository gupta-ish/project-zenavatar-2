...
class StateProcessor:
    ...
    def _prepare_low_state(self):
        ...
        for i in range(self.num_dof):
            ...
            self.tau_est[6+i] = unitree_joint_state[self.robot.JOINT2MOTOR[i]].tau_est
        
        robot_state_data = np.array(self.q.tolist() + self.dq.tolist() + self.tau_est.tolist() + self.ddq.tolist(), dtype=np.float64).reshape(1, -1)
