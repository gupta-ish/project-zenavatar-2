...
class MujocoCamera:
    def __init__(self, mj_model, mj_data, cam_id, img_shape, enable_depth=False):
        """
        mj_model: mujoco model
        mj_data: mujoco data
        cam_id: camera id in the model
        img_shape: [height, width]
        ...
        self.cam_id = cam_id
        if self.cam_id == -1:
            raise ValueError("Camera 'head_camera' not found in the model.")
        self.img_width = self.img_shape[1]; self.img_height = self.img_shape[0]
        self.cam_renderer = MujocoRenderer(self.mj_model, self.mj_data, camera_id=self.cam_id,
    ...
    def get_frame(self):
        color_frame = self.cam_renderer.render("rgb_array")
        if self.enable_depth:
            depth_frame = self.cam_renderer.render("depth_array")
            depth_frame = self.depth_2_meters(depth_frame)
        color_image = cv2.cvtColor(color_frame, cv2.COLOR_RGB2BGR)
        # color_image = color_frame
        depth_image = depth_frame if self.enable_depth else None
        return color_image, depth_image
    ...
    def depth_2_meters(self, depth):
        ...
        extend = self.mj_model.stat.extent
        near = self.mj_model.vis.map.znear * extend
        far = self.mj_model.vis.map.zfar * extend
    ...
    def __init__(self, config, port = 5555, Unit_Test = False):
        ...
        if self.head_camera_type == 'opencv':
            ...
            for serial_number in self.head_camera_id_numbers:
                ...
                self.head_cameras.append(camera)
        elif self.head_camera_type == 'mujoco':
            for cam_id in self.head_camera_id_numbers:
                camera = MujocoCamera(config['mj_model'], config['mj_data'], cam_id, self.head_image_shape)
                self.head_cameras.append(camera)
        else:
        ...
        if self.wrist_camera_type and self.wrist_camera_id_numbers:
            if self.wrist_camera_type == 'opencv':
                ...
                for serial_number in self.wrist_camera_id_numbers:
                    ...
                    self.wrist_cameras.append(camera)
            elif self.wrist_camera_type == 'mujoco':
                for model_path in self.wrist_camera_id_numbers:
                    camera = MujocoCamera(config['mj_model'], config['mj_data'], cam_id, self.wrist_image_shape)
                    self.wrist_cameras.append(camera)
            else:
        ...
        for cam in self.head_cameras:
            ...
            elif isinstance(cam, MujocoCamera):
                print(f"[Image Server] Mujoco head camera initialized with model path.")
            else:
        ...
        for cam in self.wrist_cameras:
            if isinstance(cam, OpenCVCamera):
                ...
                print(f"[Image Server] Mujoco wrist camera initialized with model path.")
            else:
        ...
        self.rate = RateLimiter(self.fps)
...
if __name__ == "__main__":
    mj_model = mujoco.MjModel.from_xml_path("/home/zenavatar/humanoidverse/humanoidverse/data/robots/g1/scene_29dof_freebase_camera.xml")
