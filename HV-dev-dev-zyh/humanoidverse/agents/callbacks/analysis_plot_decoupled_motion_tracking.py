...
class AnalysisPlotDecoupledMotionTracking(RL_EvalCallback):
    training_loop: PPO
    env: LeggedRobotBase
    ...
    def on_pre_evaluate_policy(self):
        ...
        if self.compute_metrics:
            self.pbar = tqdm(range(self.env.num_envs))
            self.success_rate = 0
            self.curr_stpes = 0
            # 
            self.success_count = 0
            self.fail_count = 0
            self.counted_envs = np.zeros(self.env.num_envs, dtype=bool)
            self.upper_body_motion_tracking_error = np.zeros(self.env.num_envs)
    ...
    def on_post_eval_env_step(self, actor_state):
        ...
        if self.compute_metrics:
            dones = actor_state['dones'].cpu().numpy().astype(bool)
            time_outs = actor_state['extras']["time_outs"].cpu().numpy()
            new_dones = dones & ~self.counted_envs

            # Count the number of successes and failures
            new_success = np.sum(new_dones & time_outs)
            new_fail = np.sum(new_dones & ~time_outs)
            self.success_count += new_success
            self.fail_count += new_fail
            
            # Update the counted_envs array
            self.counted_envs[new_dones] = True
            # import ipdb; ipdb.set_trace()
            # Calculate the upper body motion tracking error
            self.upper_body_motion_tracking_error[~self.counted_envs] += torch.sum(torch.abs(self.env.simulator.dof_pos[~self.counted_envs][:, self.env.upper_dof_indices] - \
                                                                                             self.env.ref_upper_dof_pos[~self.counted_envs][:]), dim=1).cpu().numpy()
            self.upper_body_motion_tracking_error[new_dones] /= step

            if self.pbar.n < self.pbar.total:
                # Update the pbar
                self.pbar.update(new_success + new_fail)
                self.pbar.refresh()
                total_done = self.success_count + self.fail_count
                success_rate = self.success_count / total_done if total_done > 0 else 0
                self.pbar.set_description(f"Success Rate: {self.success_count} / {total_done} ({success_rate:.2%})")
            elif not self.print_evaluation:
                print("All environments have been evaluated.")
                # import ipdb; ipdb.set_trace()
                print(f"Upper body motion tracking error: {self.upper_body_motion_tracking_error.mean():.4f}")
                self.print_evaluation = True
                self.pbar.close()
