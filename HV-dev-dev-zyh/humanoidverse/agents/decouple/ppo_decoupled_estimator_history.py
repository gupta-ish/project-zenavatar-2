...
class PPODecoupledEstimator(PPODecoupled):
    ...
    def _setup_models_and_optimizer(self):
        ...
        # Yuanhang: the estimator outputs the "scaled" force!!! (which is 0.1 * actual_force in the current setting)
    ...
    def _update_estimator(self, policy_state_dict, loss_dict):
        if self.config.update_estimator_only_when_stance:
            # Yuanhang: only use the data to update estimator when
            # 1. Stance command is 0 (0 for stance, 1 for walk)
            # 2. The apply force scale is greater than self.config.update_estimator_force_scale_threshold
            valid_idx = torch.where((
                (policy_state_dict["force_scale"] > self.config.update_estimator_force_scale_threshold) & 
                (policy_state_dict["stance_command"] == 0)))[0]
        else:
            # Yuanhang: only use the data when the apply force scale is greater than self.config.update_estimator_force_scale_threshold
            valid_idx = torch.where(
                (policy_state_dict["force_scale"] > self.config.update_estimator_force_scale_threshold))[0]
        if len(valid_idx) > 0:
            left_ee_force_estimator_target = policy_state_dict["apply_left_ee_force"][valid_idx]
            left_ee_force_estimator_output = self.left_ee_force_estimator(policy_state_dict["estimator_obs"])[valid_idx]
            right_ee_force_estimator_target = policy_state_dict["apply_right_ee_force"][valid_idx]
    ...
    def _actor_act_step(self, obs_dict):
        ...
        random_prob = torch.rand_like(obs_dict["force_scale"])  # Generate random probability
        clipped_force_scale = torch.clamp(obs_dict["force_scale"], max=self.config.maximum_force_scale_prob)  # Clip force scale probability
        if self.config.update_estimator_only_when_stance:
            estimated_force_envs_id = torch.where(
                ...
                (random_prob < clipped_force_scale)  # Apply probability filtering
            )[0]

            apply_force_envs_id = torch.where(
                (obs_dict["force_scale"] <= self.config.use_apply_or_estimated_force_scale_threshold) |
                (obs_dict["stance_command"] == 1) |
                (random_prob >= clipped_force_scale)  # Ensure disjoint sets
            )[0]
        else:
            estimated_force_envs_id = torch.where(
                (obs_dict["force_scale"] > self.config.use_apply_or_estimated_force_scale_threshold) &
        ...
        left_ee_force_estimator_output = torch.zeros((N, 3), device=self.device)
        right_ee_force_estimator_output = torch.zeros((N, 3), device=self.device)
        actor_input = torch.zeros((N,
                                   self.algo_obs_dim_dict["actor_obs"] + 
                                   self.algo_obs_dim_dict["estimated_force_history"] + 6), device=self.device)
        if len(estimated_force_envs_id) > 0:
            ...
            actor_input[estimated_force_envs_id] = torch.cat([actor_obs_estimated,
                                                              ...
                                                              right_ee_force_estimator_output[estimated_force_envs_id],
                                                              obs_dict["estimated_force_history"][estimated_force_envs_id]], dim=1)
        if len(apply_force_envs_id) > 0:
            actor_input[apply_force_envs_id] = torch.cat([actor_obs_apply,
                                                          obs_dict["apply_left_ee_force"][apply_force_envs_id],
                                                          obs_dict["apply_right_ee_force"][apply_force_envs_id],
                                                          obs_dict["apply_force_history"][apply_force_envs_id]], dim=1)
    ...
    def _pre_eval_env_step(self, actor_state: dict):
        ...
        if self.config.update_estimator_only_when_stance:
            ...
            estimated_force_history_when_stance = actor_state["obs"]["estimated_force_history"][stance_envs_idx]
            ...
            input_for_actor[stance_envs_idx] = torch.cat([actor_state["obs"]['actor_obs'][stance_envs_idx], 
                                                        ...
                                                        right_ee_force_estimator_output[stance_envs_idx],
                                                        estimated_force_history_when_stance], dim=1)
            input_for_actor[walk_envs_idx] = torch.cat([actor_state["obs"]['actor_obs'][walk_envs_idx],
                                                        actor_state["obs"]['apply_left_ee_force'][walk_envs_idx],
                                                        actor_state["obs"]['apply_right_ee_force'][walk_envs_idx],
                                                        actor_state["obs"]['apply_force_history'][walk_envs_idx]], dim=1)
        else:
            ...
            input_for_actor = torch.cat([actor_state["obs"]['actor_obs'], 
                                        ...
                                        actor_state["obs"]['estimated_force_history']], dim=1)
