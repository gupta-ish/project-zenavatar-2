...
from humanoidverse.envs.decoupled_locomotion.decoupled_locomotion_stand_height import LeggedRobotDecoupledLocomotionStanceHeight
...
DEBUG = False
class LeggedRobotDecoupledLocomotionStanceHeightForce(LeggedRobotDecoupledLocomotionStanceHeight):
    ...
    def _init_buffers(self):
        ...
        self.apply_force_phase = torch.rand((self.num_envs, 1), device=self.device)
        self.apply_force_phase_ts = torch.zeros((self.num_envs, 1), device=self.device)
        # zero force probability
        self.zero_force_prob = self.config.get("zero_force_prob", [0.2, 0.2, 0.2])
        if isinstance(self.zero_force_prob, float):
            self.zero_force_prob = [self.zero_force_prob] * 3
        self.zero_force_prob = torch.tensor(self.zero_force_prob, device=self.device)
        self.zero_force = (torch.rand((self.num_envs, 3), device=self.device) < self.zero_force_prob).float()
    ...
    def _update_apply_force_phase(self):
        ...
        self.apply_force_phase_ts[self.env_ids_stance] += 1.0 / self.apply_force_duration[self.env_ids_stance]
        ...
        self.apply_force_phase[self.env_ids_stance_non_random] = abs(torch.remainder(self.apply_force_phase_ts[self.env_ids_stance_non_random], 2.0) - 1.0)
        
        # For the random stance environments, update the phase randomly
        env_ids_stance_random_resample = self.env_ids_stance_random[torch.where(self.apply_force_phase_ts[self.env_ids_stance_random] >= 1.0)[0]]
        self.apply_force_phase[env_ids_stance_random_resample] = torch.rand((len(env_ids_stance_random_resample), 1), device=self.device)
        self.apply_force_phase_ts[env_ids_stance_random_resample] = 0.0
    ...
    def _calculate_max_ee_force(self):
        ...
        left_ee_force_phased = ee_force_min + (ee_force_max - ee_force_min) * self.apply_force_phase
        right_ee_force_phased = ee_force_min + (ee_force_max - ee_force_min) * self.apply_force_phase
        ...
        left_force_max *= (1 - self.zero_force)
        right_force_max *= (1 - self.zero_force)
    ...
    def _resample_commands(self, env_ids):
        ...
        self.apply_force_duration[env_ids] = torch.randint(self.config.randomize_force_duration[0], self.config.randomize_force_duration[1] + 1, (len(env_ids), 1), device=self.device)
        self.apply_force_phase_ts[env_ids] = torch.rand((len(env_ids), 1), device=self.device)
        self.apply_force_phase[env_ids] = torch.rand((len(env_ids), 1), device=self.device)
        self.zero_force[env_ids] = (torch.rand((len(env_ids), 3), device=self.device) < self.zero_force_prob).float()
