...
class LeggedRobotDecoupledLocomotionStanceWBC(LeggedRobotDecoupledLocomotionStance):
    ...
    def _compute_torques(self, actions):
        ...
        if self.residual_upper_body_action:
            ...
            actions_upper_body = actions_scaled[:, -self.config.robot.upper_body_actions_dim:]
        actions_scaled = torch.cat((actions_lower_body, actions_upper_body), dim=1)
    ...
    def _reward_limits_upper_body_dof_pos(self):
        ...
        out_of_limits = -(self.simulator.dof_pos[:, self.config.robot.upper_body_actions_dim:] - \
                          self.simulator.dof_pos_limits[self.config.robot.upper_body_actions_dim:, 0]).clip(max=0.)
        out_of_limits += (self.simulator.dof_pos[:, self.config.robot.upper_body_actions_dim:] - \
                          self.simulator.dof_pos_limits[self.config.robot.upper_body_actions_dim:, 1]).clip(min=0.)
        return torch.sum(out_of_limits, dim=1)
    ...
    def _reward_limits_upper_body_dof_vel(self):
        ...
        # clip to max error = 1 rad/s per joint to avoid huge penalties
        return torch.sum((torch.abs(self.simulator.dof_vel[:, self.config.robot.upper_body_actions_dim:]) - \
                          self.simulator.dof_vel_limits[self.config.robot.upper_body_actions_dim:, 1]).clip(min=0., max=1.), dim=1)
    ...
    def _reward_limits_upper_body_torque(self):
        ...
        return torch.sum((torch.abs(self.torques[:, self.config.robot.upper_body_actions_dim:]) - \
                          self.torque_limits[self.config.robot.upper_body_actions_dim:, 1]).clip(min=0.), dim=1)
    ...
    def _reward_penalty_upper_body_torques(self):
        ...
        return torch.sum(torch.square(self.torques[:, self.config.robot.upper_body_actions_dim:]), dim=1)
    ...
    def _reward_penalty_upper_body_dof_vel(self):
        ...
        return torch.sum(torch.square(self.simulator.dof_vel[:, self.config.robot.upper_body_actions_dim:]), dim=1)
    ...
    def _reward_penalty_upper_body_dof_acc(self):
        ...
        return torch.sum(torch.square((self.last_dof_vel[:, self.config.robot.upper_body_actions_dim:] - \
                                       self.simulator.dof_vel[:, self.config.robot.upper_body_actions_dim:]) / self.dt), dim=1)
    ...
    def _reward_penalty_upper_body_action_rate(self):
        ...
        return torch.sum(torch.square(self.last_actions[:, self.config.robot.upper_body_actions_dim:] - \
                                      self.actions[:, self.config.robot.upper_body_actions_dim:]), dim=1)
