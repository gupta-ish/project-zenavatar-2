...
class LeggedRobotDecoupledLocomotionStanceHeightWBCForce(LeggedRobotDecoupledLocomotionStanceHeightWBC):
    def __init__(self, config, device):
        ...
        self.left_ee_apply_force_pos_ratio = 0.5*torch.ones(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False)
        self.right_ee_apply_force_pos_ratio = 0.5*torch.ones(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        self.zero_tapping_xy_force = self.config.rewards.get("zero_tapping_xy_force", False)
    ...
    def _init_buffers(self):
        ...
        random_choices = torch.randint(0, 2, (self.num_tapping_env, 1), device=self.device)  # Random 0 or 1
    ...
    def _apply_force_in_physics_step(self):
        ...
        # TODO [Yuanhang]: Further mask where self.commands[:, :2] is zero (no movement command)
        if self.is_evaluating:
            self.walking_env_mask = (self.commands[:, 4] == 1)
            zero_cmd_mask = torch.all(self.commands[:, :2] == 0, dim=-1) 
            self.tapping_env_mask = self.walking_env_mask & zero_cmd_mask
        ...
        walking_dir[self.tapping_env_mask] = quat_rotate(self.base_quat[self.tapping_env_mask],
                                                         torch.cat([self.random_tapping_dir,
    ...
    def _pre_compute_observations_callback(self):
        ...
        left_ee_apply_force_pos = (extend_curr_pos[:, 0, :] - self.simulator._rigid_body_pos[:, self.left_hand_link_index, :]) * self.left_ee_apply_force_pos_ratio + self.simulator._rigid_body_pos[:, self.left_hand_link_index, :]
        right_ee_apply_force_pos = (extend_curr_pos[:, 1, :] - self.simulator._rigid_body_pos[:, self.right_hand_link_index, :]) * self.right_ee_apply_force_pos_ratio + self.simulator._rigid_body_pos[:, self.right_hand_link_index, :]
    ...
    #     penalty_hip_pos = torch.sum(torch.square(hip_pos), dim=1) + torch.sum(torch.abs(hip_pos), dim=1) * (1 - torch.abs(self.commands[:, 1])) 
    ...
    def _reward_base_height(self):
        ...
        # penalty_base_height[stance_env_idx] *= 5.0 # double the penalty if standing
