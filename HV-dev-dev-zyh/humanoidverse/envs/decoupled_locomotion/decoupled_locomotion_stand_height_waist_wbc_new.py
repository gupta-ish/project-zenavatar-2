...
from humanoidverse.envs.decoupled_locomotion.decoupled_locomotion_stand_waist_wbc_new import LeggedRobotDecoupledLocomotionStanceWBC
...
class LeggedRobotDecoupledLocomotionStanceHeightWBC(LeggedRobotDecoupledLocomotionStanceWBC):
    ...
    #     torso_quat_target = quat_from_euler_xyz(self.commands[:, 6], # roll
    #                                             self.commands[:, 7], # pitch
    #                                             self.commands[:, 5]) # yaw
    ...
    def _reward_penalty_torso_orientation(self):
        ...
        torso_quat = self.simulator._rigid_body_rot[:, self.torso_index]
        projected_gravity_torso = quat_rotate_inverse(torso_quat, self.gravity_vec)
        return torch.sum(torch.square(projected_gravity_torso[:, :2]), dim=1)
