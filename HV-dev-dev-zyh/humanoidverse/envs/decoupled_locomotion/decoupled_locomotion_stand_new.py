...
from humanoidverse.envs.legged_base_task.legged_robot_base_new import LeggedRobotBase
...
from humanoidverse.envs.locomotion.locomotion_new import LeggedRobotLocomotion
...
class LeggedRobotDecoupledLocomotionStance(LeggedRobotLocomotion):
    def __init__(self, config, device):
        ...
        # Create Arm IK Solver
        self.arm_ik_solver = arm_ik.G1_29_ArmIK()
    ...
    def _compute_torques(self, actions):
        ...
        """
        actions_scaled_lower_body = self.config.robot.control.action_scale * actions[:, :self.config.robot.lower_body_actions_dim]
        actions_scaled_upper_body = actions[:, -self.config.robot.upper_body_actions_dim:]
    ...
    def _reset_dofs(self, env_ids, target_state=None):
        ...
        if target_state is not None:
            self.simulator.dof_pos[env_ids, :self.config.robot.lower_body_actions_dim] = target_state[..., 0]
            self.simulator.dof_vel[env_ids, :self.config.robot.lower_body_actions_dim] = target_state[..., 1]
        else:
            self.simulator.dof_pos[env_ids, :self.config.robot.lower_body_actions_dim] = \
                self.default_dof_pos[:, :self.config.robot.lower_body_actions_dim] * \
                torch_rand_float(0.5, 1.5, (len(env_ids), self.config.robot.lower_body_actions_dim), device=str(self.device))
            self.simulator.dof_vel[env_ids, :self.config.robot.lower_body_actions_dim] = 0.
        if self.config.rewards.fix_upper_body:
            return
        ...
        self.simulator.dof_pos[env_ids, -self.config.robot.upper_body_actions_dim:] = \
            motion_res['dof_pos'][env_ids, -self.config.robot.upper_body_actions_dim:]
        self.simulator.dof_vel[env_ids, -self.config.robot.upper_body_actions_dim:] = \
            motion_res['dof_vel'][env_ids, -self.config.robot.upper_body_actions_dim:]
    ...
    def _reward_limits_dof_pos(self):
        ...
        out_of_limits = -(self.simulator.dof_pos[:, :self.config.robot.lower_body_actions_dim] - \
                          self.simulator.dof_pos_limits[:self.config.robot.lower_body_actions_dim, 0]).clip(max=0.)
        out_of_limits += (self.simulator.dof_pos[:, :self.config.robot.lower_body_actions_dim] - \
                          self.simulator.dof_pos_limits[:self.config.robot.lower_body_actions_dim, 1]).clip(min=0.)
        return torch.sum(out_of_limits, dim=1)

    def _reward_limits_dof_vel(self):
        # Penalize dof velocities too close to the limit (lower body only)
        # clip to max error = 1 rad/s per joint to avoid huge penalties
        return torch.sum((torch.abs(self.simulator.dof_vel[:, :self.config.robot.lower_body_actions_dim]) - \
                          self.simulator.dof_vel_limits[:self.config.robot.lower_body_actions_dim, 1]).clip(min=0., max=1.), dim=1)

    def _reward_limits_torque(self):
        # penalize torques too close to the limit (lower body only)
        return torch.sum((torch.abs(self.torques[:, :self.config.robot.lower_body_actions_dim]) - \
                          self.torque_limits[:self.config.robot.lower_body_actions_dim, 1]).clip(min=0.), dim=1)
    ...
    def _reward_penalty_torques(self):
        ...
        return torch.sum(torch.square(self.torques[:, :self.config.robot.lower_body_actions_dim]), dim=1)
    
    def _reward_penalty_dof_vel(self):
        # Penalize dof velocities (lower body only)
        return torch.sum(torch.square(self.simulator.dof_vel[:, :self.config.robot.lower_body_actions_dim]), dim=1)
    
    def _reward_penalty_dof_acc(self):
        # Penalize dof accelerations (lower body only)
        return torch.sum(torch.square((self.last_dof_vel[:, :self.config.robot.lower_body_actions_dim] - \
                                       self.simulator.dof_vel[:, :self.config.robot.lower_body_actions_dim]) / self.dt), dim=1)
    ...
    def _reward_penalty_stance_dof(self):
        ...
        return torch.sum(torch.square(self.simulator.dof_vel[:, :self.config.robot.lower_body_actions_dim]), dim=1) * (1.0 - self.commands[:, 4])
        ...
        return torch.sum(torch.square(self.last_dof_pos[:, :self.config.robot.lower_body_actions_dim] - \
                                      self.simulator.dof_pos[:, :self.config.robot.lower_body_actions_dim]), dim=1) * (1.0 - self.commands[:, 4])
    ...
    def _get_obs_actions(self,):
        return self.actions[:, :self.config.robot.lower_body_actions_dim]
